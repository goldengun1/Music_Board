...
Timeline::Timeline(std::shared_ptr<SoundBank> &bank, QObject *parent): QGraphicsScene(parent), bank{bank}
{
}
...
void Timeline::PaintMatrix(Matrix m)
{
	...
	while (!m.timeline.empty())
	{
		...
		m.timeline.pop();
		if (top.first.second == marktype_t::MARK_REC_STOP)
			continue;
		...
		last = new TimelineMark(0, Utils::MilisecondsToPixel(top.first.first),
		                        Utils::MilisecondsToPixel(sound.value()->Duration()),
		                        sound.value()->Source().fileName().remove(QRegExp("\\.[^.]+")));
		lastMark = top;
