...
MainWindow::MainWindow(QWidget *parent): QWidget(parent), ui(new Ui::MainWindow), bank(std::make_shared<SoundBank>())
{
	ui->setupUi(this);
...
void MainWindow::openFileDialog(SoundButton *button)
{
	QUrl qUrl = QUrl("/");
	qUrl.setScheme("file");
	auto url = QFileDialog::getOpenFileUrl(this, tr("Open audio file"), qUrl, tr("Audio files (*.wav)"));
	if (!url.isEmpty())
	{
...
void MainWindow::saveMatrix()
{
	...
	auto filePath =
	    QFileDialog::getSaveFileName(this, tr("Save Matrix"), QDir::homePath(), tr("Matrix files (*.matrix)"));
...
void MainWindow::importMatrix()
{
	...
	if (recorder->firstRecordingDuration != 0)
	{
		reply = QMessageBox::question(
		    this, "Poruka",
		    "Ovo će obrisati trenutno snimljenu matricu. Da li ste sigurni da želite da učitate matricu?");
	}

	if (reply == QMessageBox::Yes)
	{
		auto filePath =
		    QFileDialog::getOpenFileName(this, tr("Open Matrix"), QDir::homePath(), tr("Matrix files (*.matrix)"));
