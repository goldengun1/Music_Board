...
sid SoundBank::Assign(const sid index, std::shared_ptr<Sound> sound)
{
...
sid SoundBank::Assign(const sid index, const QUrl &url)
{
	auto it = this->sound.find(index);
...
std::optional<std::shared_ptr<Sound>> SoundBank::Assigned(const sid index)
{
