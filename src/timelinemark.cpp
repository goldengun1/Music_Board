...
TimelineMark::TimelineMark(int track, qreal xpos, qreal w, QString text)
    : QGraphicsItem(), color{Utils::randomColor()}, text{std::move(text)}, w{w}, track{track}, xpos{xpos}
{
...
bool TimelineMark::Overlaps(const TimelineMark *other)
{
	...
	return (this->x() < other->x() && other->x() < this->x() + w) ||
	       (this->x() < other->x() + other->w && other->x() + other->w < this->x() + w) ||
	       (other->x() < this->x() && this->x() < other->x() + other->w) ||
	       (other->x() < this->x() + w && this->x() + w < other->x() + other->w);
}
